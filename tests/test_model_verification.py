...
from tests.test_model_registry import config
...
def test_certificate_has_no_paths_or_environment(tmp_path):
    registry = ModelRegistry(tmp_path / "registry")
    registry.register_run(config(tmp_path))
    registry.create_release("run-001", "v0.6")
...
def test_training_run_can_be_verified_before_release(tmp_path):
    registry = ModelRegistry(tmp_path / "registry")
    registry.register_run(config(tmp_path))
