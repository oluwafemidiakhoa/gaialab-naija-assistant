...
class _FakeRLSNeonBackend:
    def __init__(self, database_url, migration_database_url=None):
        self.database_url = database_url; self.migration_database_url = migration_database_url; self.schema_checked = False
    def assert_schema_current(self):
        self.schema_checked = True; return {"pending": [], "drift": [], "unknown_applied_versions": []}


class _MemoryReceiptStore:
    def __init__(self): self.rows = {}
    def save(self, verification_id, envelope, *, tenant_id=None):
        self.rows[verification_id] = {"tenant_id": tenant_id, "envelope": envelope}; return True


def _clear_backend_caches():
    storage_backend.neon_backend.cache_clear(); storage_backend.operator_neon_backend.cache_clear()
...
def test_storage_mode_selects_neon_when_database_url_is_present(monkeypatch):
    ...
    monkeypatch.setattr(storage_backend, "RLSNeonBackend", _FakeRLSNeonBackend); _clear_backend_caches()
    try:
        assert storage_backend.storage_mode() == "neon"; backend = storage_backend.neon_backend()
...
def test_operator_backend_requires_separate_database_url(monkeypatch):
    ...
    try:
        ...
        try: storage_backend.operator_registry()
        except RuntimeError as exc: assert "GAIALAB_OPERATOR_DATABASE_URL" in str(exc)
        else: raise AssertionError("operator registry must fail closed without separate operator URL")
    finally: _clear_backend_caches()
...
def test_operator_backend_never_reuses_tenant_runtime_url(monkeypatch):
    ...
    monkeypatch.setattr(storage_backend, "RLSNeonBackend", _FakeRLSNeonBackend); _clear_backend_caches()
    try:
        tenant_backend = storage_backend.neon_backend(); operator_backend = storage_backend.operator_neon_backend()
...
def test_rls_neon_backend_runtime_constructor_does_not_run_schema_ddl(monkeypatch):
    def _unexpected_migration_connect(*args, **kwargs): raise AssertionError("runtime backend construction must not run migration DDL")
    monkeypatch.setattr("src.neon_storage.psycopg.connect", _unexpected_migration_connect)
    backend = RLSNeonBackend("postgresql://pooled.invalid/db", migration_database_url="postgresql://direct.invalid/db")
...
def test_neon_migrations_are_ordered_and_checksummed():
    ...
    assert [migration.name for migration in migrations] == ["initial", "tenant_rls", "database_roles", "observability", "retention_deletion", "operator_action_log"]


def test_rls_migrations_force_tenant_isolation_without_session_operator_bypass():
    migrations = discover_migrations(); rls_sql = migrations[1].sql; role_sql = migrations[2].sql
    observability_sql = migrations[3].sql; retention_sql = migrations[4].sql; action_sql = migrations[5].sql
    ...
    assert "SESSION_USER" in role_sql and "gaialab_is_operator()" in role_sql and "gaialab.operator_mode" not in role_sql
    assert "gaialab_current_role_kind" in observability_sql
    assert "retention_deletion_plans" in retention_sql and "verification_receipts" not in retention_sql
    assert "operator_action_log_heads" in action_sql and "operator_actions" in action_sql and "action_hash" in action_sql


def test_verify_payload_accepts_injected_production_store():
    store = _MemoryReceiptStore()
    result = verify_payload({"assistant_response": "The transfer is still pending.", "authoritative_state": {"transaction_status": "pending"}, "assistant_claims": {"transaction_status": "pending"}, "model_name": "example-model"}, tenant_id="tenant_neon", receipt_store_backend=store)
...
def test_sqlite_remains_default_without_neon_url(monkeypatch):
    monkeypatch.delenv("GAIALAB_DATABASE_URL", raising=False); _clear_backend_caches()
    try: assert storage_backend.storage_mode() == "sqlite"
    finally: _clear_backend_caches()
